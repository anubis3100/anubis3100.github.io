...
// ── intro splash ───────────────────────────────────────────────────────────
(function intro() {
  // show on first visit of this tab (use sessionStorage so refresh re-shows it only once per tab)
  // To show every visit instead, remove the sessionStorage guard.
  if (sessionStorage.getItem('introSeen') === '1') return;

  const pool = [
    ...digitalWorks.map((w, i) => ({ route: 'digital',  index: i, src: 'assets/digital work/'       + (typeof w === 'string' ? w : w.file) + '?v=4' })),
    ...physicalWorks.map((w, i) => ({ route: 'physical', index: i, src: 'assets/physical paintings/' + (typeof w === 'string' ? w : w.file) + '?v=4' })),
    ...studiesWorks.map((w, i) => ({ route: 'studies',  index: i, src: 'assets/studies/'             + (typeof w === 'string' ? w : w.file) + '?v=4' })),
  ];
  ...
  // pre-placed slots around the central name — roughly elliptical scatter
  // each slot: x (vw %), y (vh %), size (px), rot (deg)
  const slots = [
    { x: 10,  y: 18, size: 130, rot: -6 },
    { x: 26,  y: 70, size: 150, rot:  4 },
    { x: 16,  y: 44, size: 120, rot:  8 },
    { x: 36,  y: 12, size: 110, rot: -4 },
    { x: 82,  y: 22, size: 140, rot:  6 },
    { x: 74,  y: 68, size: 160, rot: -5 },
    { x: 88,  y: 50, size: 120, rot:  2 },
    { x: 60,  y: 84, size: 130, rot: -7 },
    { x: 42,  y: 88, size: 110, rot:  5 },
    { x: 6,   y: 80, size: 100, rot: -3 },
  ];

  const overlay = document.createElement('div');
  overlay.id = 'intro';
  overlay.innerHTML = `
    <div class="intro-artworks"></div>
    <div class="intro-center">
      <h1 class="intro-name">anubis3100</h1>
      <p class="intro-hint">click to enter</p>
    </div>
  `;
  document.body.appendChild(overlay);

  const aw = overlay.querySelector('.intro-artworks');
  const center = overlay.querySelector('.intro-center');

  let poolCursor = 0;
  function pickNext() {
    const it = pool[poolCursor % pool.length];
    poolCursor++;
    return it;
  }

  const slotEls = slots.map((slot, i) => {
    const el = document.createElement('button');
    el.className = 'intro-art';
    el.type = 'button';
    ...
    el.style.setProperty('--float-delay', (i * 0.37).toFixed(2) + 's');
    el.style.setProperty('--float-dur',  (9 + (i % 4)).toFixed(1) + 's');
    ...
    el.addEventListener('mousemove', (e) => {
      ...
      el.style.setProperty('--glow-x', (-px * 30) + 'px');
      el.style.setProperty('--glow-y', (-py * 30 + 18) + 'px');
    });
    el.addEventListener('mouseleave', () => {
      el.style.setProperty('--tilt-x', '0deg');
      el.style.setProperty('--tilt-y', '0deg');
      el.style.setProperty('--glow-x', '0px');
      el.style.setProperty('--glow-y', '20px');
    });
    el.addEventListener('click', (e) => {
      e.stopPropagation();
      const data = el._data;
      if (!data) return closeIntro('digital');
      closeIntro(data.route, data.index);
    });
    aw.appendChild(el);
    return el;
  });

  // assign initial artworks + start cycling
  function assignTo(el, data) {
    el._data = data;
    const img = el.querySelector('img');
    img.src = data.src;
  }
  slotEls.forEach(el => assignTo(el, pickNext()));

  // cycle: swap one slot every ~1.6s with a crossfade
  const cycleTimer = setInterval(() => {
    const idx = Math.floor(Math.random() * slotEls.length);
    const el = slotEls[idx];
    el.classList.add('swapping');
    setTimeout(() => {
      assignTo(el, pickNext());
      el.classList.remove('swapping');
    }, 420);
  }, 1600);

  // click on name or empty overlay background → digital
  overlay.addEventListener('click', (e) => {
    if (e.target.closest('.intro-art')) return;
    closeIntro('digital');
  });
  // enter key / any key → digital
  function onKey(e) {
  ...
  function closeIntro(route, index) {
    sessionStorage.setItem('introSeen', '1');
    clearInterval(cycleTimer);
    document.removeEventListener('keydown', onKey);
    if (typeof index === 'number') {
      window.__introTarget = { route, index };
    }
    ...
    setTimeout(() => {
      ...
      if (route !== getRoute()) {
        ...
        // force re-render to pick up target index
        renderRoute(route);
      }
    }, 520);
  }
